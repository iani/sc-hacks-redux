...
~b = { SinOsc.ar(2000, 0, 0.1).dup }.play;
~b = Pbind(\amp, 0.4, \degree, Pwhite(10, 20)).play;
...
To add variables, either one has to edit the source code of the class concerned, or one has to create a subclass of that class, and add the variables to the subclass.  This has several disadvantages or problems:
  - Adding variables and/or subclasses leads to bloating of the code and can make it difficult to follow, because the roles of variables and their interdepences become more complex.
  - One may not want to modify the code of classes defined by other programmers because they belong to libraries whose modification requires checking by the authors.

One way to add state to an object without adding variables, is to store its state in a dictionary and access it using the object as key.  A good example of this pattern is the dependant pattern in SuperCollider.  This uses classvar dependantsDictionary of Object in order to store the dependants of an Object.  The 
