...
GrainBuf0 with limited amplitude control (amplifyl)
and limiter, to prevent clipping.
...
GrainBuf0l_ : UGenFunc {
	*ar { | numChannels = 1, limlim = 0.5 |
		var trig, dur, buf, rate, pos, pan, src;
		trig = \trig.br(0);
		// trig = \trig.tr(0);
		dur = \dur.br(~dur ? 1);
		...
		src = GrainBuf.ar(
			...
			512 // maxGrains
		) * \vol.br(~vol ? 1).amplifyl(0.2);
		^Limiter.ar(src, limlim); // dur = 0.01 (default.)
	}
}
