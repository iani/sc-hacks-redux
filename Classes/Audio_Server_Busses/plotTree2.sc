/*  6 Dec 2022 14:29
A Server plotTree that displays group names.
*/

+ Server {
	plotTree2 {|interval=0.5|
		var onClose, window = Window.new(name.asString + "Node Tree",
			Rect(128, 64, 400, 400),
			scroll:true
		).front;
		window.view.hasHorizontalScroller_(false).background_(Color.grey(0.9));
		onClose = this.plotTreeView2(interval, window.view, { defer {window.close}; });
		window.onClose = {
			onClose.value;
		};
	}

	plotTreeView2 {|interval=0.5, parent, actionIfFail|
		var resp, done = false;
					...
					if(node.value.isArray, {
						...
						pen.stringInRect(
							...
							if (node.key == 1) {
								...
								"-" + Groups.names[node.key]
							},
							//						(node.key == 1).if("- default group", { node.asString }),
							rect
						);
