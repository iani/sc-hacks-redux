...
TypingSynthdefs {
	...
	*loadAll {
		...
		[\sline1, \sline2, \sline3, \sline3b,
			\sline4, \sline4b, \sline5] do: { | d |
			postln("Loading" + d + "...");
	...
	*sline3b {
		^SynthDef(\sline3b, { | out = 0, ffreq=0.3, ascii = 50,
			startfreq = 200, endfreq = 800, dur = 1,
			amp = 0.1, mul = 1 |
			var src, env, freq, lfo, noise, decay;
			lfo = SinOsc.kr(ffreq).exprange(0.3,0.9);
			noise = WhiteNoise.ar(1);
			...
			// dur = Rand(0.2, 1.8);
			// amp = Rand(0.01, 0.1);
			freq = Line.kr(startfreq, endfreq, dur,
				doneAction: 2
			);
			env = Env.perc(0.1, dur - 0.1);
			src = Ringz.ar(
	...
	*sline4b {
		^SynthDef(\sline4b, { | ascii = 50, dur = 1 |
			var src, env, freq, amp, pan;
			ascii = ascii.wrap(1, 127).linlin(1, 127, 1.01, 2);
			// dur = Rand(0.1, 0.8);
			amp = Rand(0.01, 0.1);
			freq = Line.kr(
				Rand(100, 1000),
				Rand(100, 1000) * (1..10).normalize(1, ascii),
				dur,
				doneAction: 2
			);
			env = Env.perc(0.01, dur - 0.01, amp / 10);
			freq = freq * (1..10).normalize(1, ascii);
			src = Mix(SinOsc.ar(freq));
			pan = Pan2.ar(src, Rand(-1.0, 1.0), env.kr(1, doneAction: 2));
			Out.ar(0, pan);
		});
	}
