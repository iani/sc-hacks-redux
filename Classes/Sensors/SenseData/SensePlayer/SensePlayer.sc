...
SensePlayer : NamedSingleton {
	...
	setLocalPort { | portNum = 10000 |
		postf("Setting local port number to %\n", portNum);
		addr = NetAddr("127.0.0.1", portNum);
		postf("My sending address number is now: %\n", addr);
	}
	...
	readDialog { // Select file with dialog window
